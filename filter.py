...
def filter_graph(graph, valid_nodes):
	...
	for n in graph['batadv']['nodes']:
		if 'node_id' not in n or valid_nodes[n['node_id']]:
			pos_map[orig_pos] = new_pos
			new_pos += 1
		orig_pos += 1

	nodes_ffv = filter(lambda n:'node_id' not in n or valid_nodes[n['node_id']], graph['batadv']['nodes'])
...
def filter_json(graph, nodes, nodelist):
	...
	# force Pl-Garten5-OG to be part of FFV
	valid_nodes['a0f3c1984ca8'] = True
